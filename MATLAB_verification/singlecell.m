...
% Updated for 3 nodes wide and 3 nodes tall structure with 10 kN loads
% Node numbering for 3x3 grid (Nx=2, Ny=2):
% 1 -- 2 -- 3
% |    |    |
% 4 -- 5 -- 6
% |    |    |
...
LoadNodes = [3, 6, 9]; % Applying loads on the top row nodes
LoadMagnitudes = [10, 10, 10]; % 10 kN loads

% Generate the structured orthogonal domain
% Updated Nx=2 (3 nodes wide), Ny=2 (3 nodes tall), Lx=2, Ly=2
[NODE, ELEM, SUPP, LOAD, BARS] = StructDomain(2, 2, 2, 2, 'bridge', LoadNodes, LoadMagnitudes);
...
% Updated movable nodes based on new 3x3 structure
movableNodesX = [2, 5, 8]; % Middle column nodes (nodes 2, 5, 8)
movableNodesY = [4, 5, 6]; % Middle row nodes (nodes 4, 5, 6)
